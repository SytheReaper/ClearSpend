...
import pandas as pd
import numpy as np
import json, os, math, calendar, re, io, base64
...
# ── Display currency ─────────────────────────────────────────────────────────
# Everything is STORED in USD. The user can choose a display currency; the
# usd()/usd0()/kfmt() formatters convert USD→display on the fly. The AI plumbing
# uses the us*u() variants, which always stay in USD to match storage.
CUR_RATES = {"USD": 1.0}
for _cd in CITY_DATA.values():
    CUR_RATES.setdefault(_cd["cur"], _cd["fx"])
CUR_SYM = {"USD":"$","GBP":"£","EUR":"€","SGD":"S$","TRY":"₺","SAR":"SR ","AED":"AED ","INR":"₹"}
def cur_sym(c): return CUR_SYM.get(c, c + " ")
def _disp():
    c = globals().get("DISP_CUR", "USD")
    return CUR_RATES.get(c, 1.0), cur_sym(c)

def usdu(v):  return f"${v:,.2f}"   # always USD (AI context / storage anchor)
def usd0u(v): return f"${v:,.0f}"
def usd(v):
    r, s = _disp(); return f"{s}{v*r:,.2f}"
def usd0(v):
    r, s = _disp(); return f"{s}{v*r:,.0f}"
def kfmt(v):
    r, s = _disp(); x = v*r; a = abs(x)
    if a >= 1_000_000: return f"{s}{x/1_000_000:.2f}M"
    if a >= 1_000:     return f"{s}{x/1_000:.1f}k"
    return f"{s}{x:,.0f}"
...
# ── Hijri (Islamic) calendar — tabular conversion, no external libs ───────────
...
def greg_to_hijri(g):
    """Approximate Gregorian→Hijri (tabular Islamic calendar). Returns (y, m, d)."""
    jdn = _greg_jdn(g.year, g.month, g.day)
    l = jdn - 1948440 + 10632
    n = (l - 1) // 10631
    l = l - 10631*n + 354
    ...
    return (y, m, d)
def hijri_str(g):
    y, m, d = greg_to_hijri(g)
    return f"{d} {HIJRI_MONTHS[max(0,min(11,m-1))]} {y} AH"
# ════════════════════════════════════════════════════════════════════════════
...
def render_svg(svg, height):
    """Render an SVG reliably inside the page.
    st.markdown mangles long/complex SVG strings (they leak as raw text), so we
    embed the SVG in a tiny transparent HTML document via components.html."""
    html = (f'<div style="width:100%;display:flex;justify-content:center;'
            f'font-family:Inter,sans-serif">{svg}</div>')
...
def empty_data():
    """A blank starting state — no transactions, no assets."""
    return {
        "entries":[], "assets":[], "liabilities":[], "goals":[],
        "budgets":{}, "city":"Riyadh", "history":[],
        "zakat":{"hawl_start":None, "gold_price_g":80.0, "payments":[]},
    }

def sample_data():
    """Optional demo dataset, loaded only when the user clicks 'Load sample data'."""
    today = date.today()
    ...
    return {
        ...
        "city":"Riyadh","history":[],
        "zakat":{"hawl_start":str(today-timedelta(days=200)),"gold_price_g":80.0,"payments":[]},
    }
...
def load():
    if os.path.exists(DATA_FILE):
        try:
            ...
            z = d.get("zakat") or {}
            z.setdefault("hawl_start", None); z.setdefault("gold_price_g", 80.0)
            z.setdefault("payments", []); d["zakat"] = z
            return d
        except Exception:
            return empty_data()
    return empty_data()
...
def snapshot():
    """Remember the current data so the last change can be undone (one level)."""
    try: st.session_state.undo = json.dumps(st.session_state.data)
...
# Display currency: defaults to the selected city's currency (e.g. SAR for Riyadh).
DISP_CUR = D.get("disp_cur", CITY_DATA.get(city, {}).get("cur", "USD"))
if DISP_CUR not in CUR_RATES: DISP_CUR = "USD"
...
SAUDI_SAVINGS = {
 "car_insurance": {
   ...
   "intro":"This is the single biggest 'same product, wildly different price' cost in Saudi. Identical cover for the same car can vary 30–60% between insurers, so NEVER auto-renew — always re-quote on the aggregators first. Comprehensive (شامل) covers your own car too; Third-Party (ضد الغير) only covers damage you cause to others.",
   "options":[
     {"name":"Third-Party (TPL) — minimum legal","price":(300,900),"tag":"Cheapest","note":"The legal minimum. Best for older / low-value cars (8+ yrs). Covers others' damage only, not your own car."},
     {"name":"TPL+ / 'third party plus'","price":(700,1500),"tag":"Cheapest","note":"TPL with limited own-damage extras (fire/theft) — a cheap middle ground some insurers offer."},
     {"name":"Walaa / ACIG comprehensive","price":(1300,2900),"tag":"Cheapest","note":"Frequently the lowest comprehensive (شامل) quotes for the same cover on Tameeni."},
     {"name":"Salama / Wataniya / Gulf Union","price":(1450,3100),"tag":"Value","note":"Competitive takaful pricing, decent repair networks."},
     {"name":"Al Rajhi Takaful comprehensive","price":(1600,3400),"tag":"Value","note":"Popular Shariah-compliant option, smooth online renewal in the app."},
     {"name":"Malath / Saudi Re / Arabian Shield","price":(1700,3500),"tag":"Standard","note":"Mid-market insurers — worth quoting, sometimes undercut the big names."},
     {"name":"Tawuniya comprehensive","price":(1800,3900),"tag":"Popular","note":"Largest insurer, widest agency-repair network and roadside cover."},
     {"name":"Bupa / MedGulf comprehensive","price":(1900,4100),"tag":"Standard","note":"Solid mid-to-upper pricing, reliable claims service."},
     {"name":"GIG / Allianz Saudi Fransi","price":(2000,4800),"tag":"Premium","note":"Higher cost; premium agency repair, courtesy car and faster service."},
   ],
   "tips":[
     "Re-quote on Tameeni AND Najm (tameeni.com / najm.sa) at every renewal — same car, very different prices. Take screenshots to compare.",
     "Raise your deductible/excess (تحمل) — accepting a higher excess can cut the premium 15–30%.",
     "If your car is old or worth little, drop comprehensive and take TPL only — the شامل premium can exceed the car's value.",
     "Bundle multiple cars or family policies with one insurer for a loyalty discount.",
     "A clean no-claims record earns a big discount — don't claim for tiny scratches you can fix cheaply.",
     "Pay annually, not in instalments — instalment plans add a financing fee.",
     "Decline add-ons you won't use (agency repair on an old car, GCC cover if you never drive abroad).",
     "Quote 2–3 weeks BEFORE expiry; last-minute renewals lose you negotiating room.",
   ],
   "save_est":160,
 },
 "car_maintenance": {
 ...
 "health_insurance": {
   ...
   "intro":"If you buy your own cover (not employer-provided), the network 'class' (A/A+/VIP) and the co-pay drive the price far more than the brand. Match the class to the hospitals you'd actually use — paying for a VIP network you never visit is pure waste.",
   "options":[
     {"name":"Basic / Class C network plan","price":(1500,3000),"tag":"Cheapest","note":"Covers essentials at a smaller hospital network. Fine if you're young and healthy."},
     {"name":"Class B network (Bupa/Tawuniya)","price":(2800,5000),"tag":"Value","note":"Wider network with reasonable co-pay — the sweet spot for most families."},
     {"name":"Class A network","price":(4500,8000),"tag":"Standard","note":"Most private hospitals included, lower co-pay."},
     {"name":"MedGulf / Malath mid plan","price":(2800,5200),"tag":"Value","note":"Worth quoting against Bupa/Tawuniya — often cheaper for the same class."},
     {"name":"Premium VIP / A+ plan","price":(6000,13000),"tag":"Premium","note":"Top hospitals, low/no co-pay, dental & maternity extras."},
   ],
   "tips":[
     "Pick the lowest network class that includes the hospitals near you — don't pay for A+ if your clinic is in class B.",
     "A small co-pay (e.g. 20% on outpatient) dramatically lowers the premium and discourages over-use.",
     "Compare on aggregators (Tameeni, Bcare, durah) at every renewal — quotes for the same cover vary a lot.",
     "Add maternity/dental only the year you'll actually use them; drop them otherwise.",
     "If employed, check what your employer plan already covers before buying extra family cover.",
   ],
   "save_est":140,
 },
 "life_insurance": {
   "label":"Life / Protection Insurance","emoji":"🛡️","maps":None,"unit":"per year",
   "intro":"Useful if people depend on your income (family, mortgage). Term/takaful protection is cheap; avoid bundling it with savings 'investment' products, which carry high hidden fees.",
   "options":[
     {"name":"Term takaful (pure protection)","price":(400,1500),"tag":"Cheapest","note":"Pays a lump sum if you pass away within the term. Cheapest, no frills — what most people actually need."},
     {"name":"Mortgage/loan protection","price":(500,2000),"tag":"Value","note":"Clears your mortgage/loan if you die. Often required by the bank; shop it separately, don't just take the bank's."},
     {"name":"Whole-of-life / savings-linked","price":(3000,12000),"tag":"Premium","note":"Bundles investing with cover — high fees, hard to exit. Usually better to buy term + invest the difference."},
   ],
   "tips":[
     "Buy term takaful and invest the difference yourself — far cheaper than bundled 'savings' policies.",
     "Only insure for as long as people depend on you (until kids are grown / mortgage is paid).",
     "Shop bank loan-protection cover independently — the bank's default is rarely the cheapest.",
     "Don't over-insure: cover income replacement + debts, not a random round number.",
   ],
   "save_est":60,
 },
 "electronics": {
 ...
 "water": {
   "label":"Water Bill","emoji":"🚰","maps":"utilities","unit":"per month",
   "intro":"Riyadh water is supplied by NWC at subsidised residential rates, so the bill is almost entirely about consumption — leaks and irrigation are the usual culprits.",
   "options":[
     {"name":"Efficient household use","price":(0,0),"tag":"Cheapest","note":"Fix leaks, low-flow fittings — keeps you in the low tariff band."},
     {"name":"High use (garden/irrigation)","price":(0,0),"tag":"Costly","note":"Outdoor watering and leaks push consumption up fast."},
   ],
   "tips":[
     "Check for silent leaks: read the meter at night, again in the morning — movement with all taps off means a leak.",
     "Fit low-flow showerheads and dual-flush toilets; they pay back in a couple of months.",
     "Water the garden at night to cut evaporation, and switch to drip irrigation.",
     "Bulk-buy bottled water from Othaim/Panda or use a home filter instead of small bottles.",
   ],
   "save_est":40,
 },
 "domestic_help": {
   "label":"Domestic Help","emoji":"🧹","maps":None,"unit":"per service / month",
   "intro":"Hourly on-demand cleaning is convenient but adds up. Compare it honestly against the all-in cost of a sponsored worker if you need help daily.",
   "options":[
     {"name":"On-demand apps (per visit)","price":(80,200),"tag":"Value","note":"Mr&Mrs / OWNA / app cleaners — pay only when you need it. Best for occasional help."},
     {"name":"Part-time recurring cleaner","price":(600,1500),"tag":"Standard","note":"A fixed weekly slot, often cheaper per hour than ad-hoc bookings."},
     {"name":"Full-time sponsored worker (all-in)","price":(2000,3500),"tag":"Premium","note":"Salary + fees + food/housing. Only worth it for daily, full-time need."},
   ],
   "tips":[
     "For a few hours a week, on-demand apps beat the all-in cost of a full-time worker.",
     "Book recurring slots — providers discount regular weekly bookings vs one-offs.",
     "Compare Mr&Mrs vs OWNA vs local agencies; per-hour rates differ noticeably.",
     "Share a trusted cleaner with neighbours to fill their schedule and lower your rate.",
   ],
   "save_est":120,
 },
 "kids_school": {
   "label":"Kids' Schooling & Activities","emoji":"🎒","maps":"education","unit":"per year",
   "intro":"International schools in Riyadh range enormously in price for similar curricula. Tuition is only part of it — buses, uniforms, books and activities stack up.",
   "options":[
     {"name":"Public / government school","price":(0,0),"tag":"Cheapest","note":"Free for eligible families."},
     {"name":"Mid-tier private (Arabic/Intl curriculum)","price":(8000,25000),"tag":"Value","note":"Solid academics at a fraction of premium-school fees."},
     {"name":"Premium international school","price":(40000,90000),"tag":"Premium","note":"British/American/IB brands — you pay for facilities and brand."},
   ],
   "tips":[
     "Two schools with the same curriculum (e.g. British) can differ 2–3× in fees — shortlist and compare.",
     "Pay tuition annually or per the early-payment discount many schools offer.",
     "Buy uniforms a size up and reuse; source books second-hand from parent groups.",
     "Sibling discounts exist at most private schools — always ask admissions.",
     "Free/low-cost activities (public libraries, sports clubs, municipality programs) rival paid ones.",
   ],
   "save_est":300,
 },
 "flights_travel": {
   "label":"Flights & Travel","emoji":"✈️","maps":None,"unit":"per trip",
   "intro":"From Riyadh (RUH), budget carriers and flexible dates beat full-service fares for most regional trips. Booking timing matters more than loyalty.",
   "options":[
     {"name":"Budget carriers (flyadeal, flynas)","price":(0,0),"tag":"Cheapest","note":"Cheapest for domestic & regional routes — pack light to skip bag fees."},
     {"name":"Full-service economy (Saudia)","price":(0,0),"tag":"Standard","note":"More baggage and comfort; bundle with Alfursan miles."},
     {"name":"Premium / last-minute","price":(0,0),"tag":"Premium","note":"Flexible/business fares — by far the priciest."},
   ],
   "tips":[
     "Compare on Google Flights / Wego and book 4–8 weeks out; mid-week flights are cheaper.",
     "Fly budget carriers (flyadeal, flynas) for short hauls and travel cabin-bag only.",
     "Use a fare-alert and be flexible by a day or two around holidays — prices swing hugely.",
     "Pay in the local currency, and use a card with no FX fee for overseas bookings.",
   ],
   "save_est":80,
 },
 "leisure": {
   "label":"Entertainment & Outings","emoji":"🎟️","maps":"entertainment","unit":"per outing",
   "intro":"Riyadh has exploded with paid events (Season, theme parks, cinema). Easy to overspend — plan around free and discounted options.",
   "options":[
     {"name":"Free & public (parks, walks, libraries)","price":(0,30),"tag":"Cheapest","note":"Salam Park, Wadi Hanifah, public events during Riyadh Season."},
     {"name":"Cinema / casual outings","price":(40,120),"tag":"Value","note":"Use cinema loyalty (VOX/Muvi) and weekday/early shows."},
     {"name":"Theme parks / big events","price":(150,500),"tag":"Premium","note":"Boulevard, Winter Wonderland — buy bundles, avoid peak nights."},
   ],
   "tips":[
     "Buy event/theme-park tickets in advance bundles — gate prices and peak nights cost far more.",
     "Use cinema loyalty apps and weekday matinee pricing.",
     "Mix in free outings (parks, free Season zones, public art) between paid ones.",
     "Set a monthly 'fun' budget in the Budgets tab so outings don't quietly blow the month.",
   ],
   "save_est":90,
 },
}
...
# Fast & low-cost — good for a shared key. For stronger answers swap to a Sonnet
# or Opus model, e.g. "claude-sonnet-4-6" or "claude-opus-4-8".
CLAUDE_MODEL = "claude-haiku-4-5"

def _clean_key(k):
    """Sanitise an API key so a stray smart-quote / space / wrapping quote that
    got pasted in can't crash the HTTP request ('latin-1' codec errors) or cause
    a bogus 401. Headers must be ASCII, so strip anything that isn't."""
    if not k:
        return ""
    k = str(k).strip()
    # remove surrounding straight or smart quotes
    k = k.strip('"\'' + "‘’“”").strip()
    # drop any non-ASCII characters (e.g. a curly apostrophe pasted mid-key)
...
def build_finance_context():
    ...
    All amounts are reported in USD (the storage currency) to keep the AI anchored,
    regardless of the user's chosen display currency."""
    lines = []
    lines.append(f"User location: {city} (local currency {CITY_DATA[city]['cur']}, "
                 f"display currency {DISP_CUR}). All amounts below are in USD.")
    lines.append(f"Monthly income: {usdu(mo_inc)}. Monthly expenses: {usdu(mo_exp)}. "
                 f"Net per month: {usdu(net)}. Savings rate: {sav_rate:.1f}%.")
    lines.append(f"Net worth: {usdu(net_worth)} (assets {usdu(total_assets)}, liabilities {usdu(total_liabs)}). "
                 f"Debt-to-income: {dti:.1f}%. Spare cash after debt: {usdu(spare)}/mo. "
                 f"Financial health score: {HS}/100 (grade {grade}).")
    if cat_tot:
        cats = ", ".join(f"{CATEGORIES.get(k,CATEGORIES['other'])['label']} {usdu(v)}/mo"
                         for k,v in sorted(cat_tot.items(), key=lambda x:-x[1]))
        lines.append("Spending by category: " + cats + ".")
    if budgets:
        bd = ", ".join(f"{CATEGORIES.get(k,CATEGORIES['other'])['label']} {usdu(cat_tot.get(k,0))}/{usdu(v)}"
                       for k,v in budgets.items())
        lines.append("Budgets (spent/limit): " + bd + ".")
    if goals:
        gl = ", ".join(f"{g['name']} {usdu(g['current'])}/{usdu(g['target'])}" for g in goals)
    ...
    # Transaction list with ids so the assistant can update/delete precisely.
    if entries:
        lines.append("Transactions (id · name · amount · freq · category · type):")
        for e in sorted(entries, key=lambda x:-monthly_eq(x["amount"],x["ptype"]))[:40]:
            lines.append(f"  #{e['id']} {e['name']} {usdu(e['amount'])} {e['ptype']} "
                         f"{e['category']} {e['type']}")
    return "\n".join(lines)
...
# ── AI TOOLS  ────────────────────────────────────────────────────────────────
# The assistant can ACTUALLY change your data (not just talk about it). These
# tool definitions are exposed to the model; when it calls one, ai_execute()
# mutates the live data and saves it, and the app reruns so every tab updates.
def _cat_enum():  return list(CATEGORIES.keys())
def _freq_enum(): return list(PAYMENT_TYPES.keys())

def ai_tool_defs():
    """Neutral tool spec (JSON-schema style) shared by Claude & Gemini."""
    cat, freq = _cat_enum(), _freq_enum()
    return [
        {"name":"add_transaction",
         "description":"Add a new expense or income entry to the user's tracker. Amounts are in USD. Use this whenever the user asks to add, log, record or create a transaction.",
         "input_schema":{"type":"object","properties":{
            "name":{"type":"string","description":"Short description, e.g. 'Car insurance'"},
            "amount":{"type":"number","description":"Amount in USD (positive number)"},
            "type":{"type":"string","enum":["expense","income"]},
            "category":{"type":"string","enum":cat,"description":"Spending category. Omit to auto-detect."},
            "frequency":{"type":"string","enum":freq,"description":"monthly, single (one-time), weekly, biweekly, yearly or daily. Default monthly for bills, single for one-offs."},
            "date":{"type":"string","description":"ISO date YYYY-MM-DD. Default today."},
            "note":{"type":"string"}},
            "required":["name","amount","type"]}},
        {"name":"update_transaction",
         "description":"Change an existing transaction's amount, name, category, frequency or note. Identify it by id (preferred) or by a name to match.",
         "input_schema":{"type":"object","properties":{
            "id":{"type":"integer","description":"Transaction id from the snapshot."},
            "match_name":{"type":"string","description":"If no id, the name (or part of it) to find."},
            "amount":{"type":"number"},"name":{"type":"string"},
            "category":{"type":"string","enum":cat},"frequency":{"type":"string","enum":freq},
            "note":{"type":"string"}},
            "required":[]}},
        {"name":"delete_transaction",
         "description":"Remove a transaction by id or by matching its name.",
         "input_schema":{"type":"object","properties":{
            "id":{"type":"integer"},"match_name":{"type":"string"}},"required":[]}},
        {"name":"set_budget",
         "description":"Set or update a monthly spending limit (USD) for a category.",
         "input_schema":{"type":"object","properties":{
            "category":{"type":"string","enum":cat},"limit":{"type":"number"}},
            "required":["category","limit"]}},
        {"name":"add_goal",
         "description":"Create a savings goal.",
         "input_schema":{"type":"object","properties":{
            "name":{"type":"string"},"target":{"type":"number"},"current":{"type":"number"}},
            "required":["name","target"]}},
        {"name":"add_asset",
         "description":"Add something the user owns (cash, investments, property, crypto, vehicle) to their net worth.",
         "input_schema":{"type":"object","properties":{
            "name":{"type":"string"},
            ...
            "required":["name","value"]}},
        {"name":"add_liability",
         "description":"Add a debt (loan, mortgage, credit card) to the user's net worth.",
         "input_schema":{"type":"object","properties":{
            "name":{"type":"string"},
            "type":{"type":"string","enum":LIABILITY_TYPES},
            "balance":{"type":"number","description":"Outstanding balance in USD"},
            "monthly_payment":{"type":"number","description":"Monthly payment in USD"},
            "note":{"type":"string"}},
            "required":["name","balance"]}},
        {"name":"find_cheaper_alternative",
         "description":"Look up real cheaper alternatives and money-saving options for a common Saudi cost (e.g. car insurance, mobile plan, gym). Returns ranked providers/options with indicative SAR prices and saving tips. Use this when the user wants to spend less on something or asks for a better/cheaper alternative.",
         "input_schema":{"type":"object","properties":{
            "topic":{"type":"string","enum":list(SAUDI_SAVINGS.keys()),
                     "description":"Which cost to find alternatives for."}},
            "required":["topic"]}},
    ]

def ai_execute(name, args):
    """Run a tool the model requested. Mutates D and returns a short result string."""
    args = args or {}
    try:
        if name == "add_transaction":
            nm = str(args.get("name","")).strip()
            amt = float(args.get("amount",0) or 0)
            typ = args.get("type","expense")
            if typ not in ("expense","income"): typ = "expense"
            cat = args.get("category") or detect_category(nm, typ)
            if cat not in CATEGORIES: cat = detect_category(nm, typ)
            freq = args.get("frequency","monthly")
            if freq not in PAYMENT_TYPES: freq = "monthly"
            dt = str(args.get("date") or date.today().isoformat())
            if not nm or amt <= 0:
                return "ERROR: a name and a positive amount are required."
            entries.append({"id":max((x["id"] for x in entries),default=0)+1,"name":nm,
                "amount":amt,"type":typ,"ptype":freq,"category":cat,"date":dt,
                "note":str(args.get("note","") or "added by AI"),"linked_goal":None})
            save(D)
            return f"✓ Added {typ} '{nm}' — {usdu(amt)} ({PAYMENT_TYPES[freq]['label']}, {CATEGORIES[cat]['label']})."
        if name == "update_transaction":
            tgt = None
            if args.get("id") is not None:
                tgt = next((e for e in entries if e["id"]==int(args["id"])), None)
            if tgt is None and args.get("match_name"):
                q = str(args["match_name"]).lower()
                tgt = next((e for e in entries if q in e["name"].lower()), None)
            if tgt is None:
                return "ERROR: no matching transaction found."
            changed = []
            if args.get("amount") is not None:
                tgt["amount"] = float(args["amount"]); changed.append(f"amount→{usdu(tgt['amount'])}")
            if args.get("name"):
                tgt["name"] = str(args["name"]).strip(); changed.append("name")
            if args.get("category") in CATEGORIES:
                tgt["category"] = args["category"]; changed.append("category")
            if args.get("frequency") in PAYMENT_TYPES:
                tgt["ptype"] = args["frequency"]; changed.append("frequency")
            if args.get("note") is not None:
                tgt["note"] = str(args["note"]); changed.append("note")
            save(D)
            return f"✓ Updated '{tgt['name']}' ({', '.join(changed) or 'no fields'})."
        if name == "delete_transaction":
            ...
            if tgt is None:
                return "ERROR: no matching transaction found."
            entries[:] = [e for e in entries if e["id"]!=tgt["id"]]; save(D)
            return f"✓ Deleted '{tgt['name']}' ({usdu(tgt['amount'])})."
        if name == "set_budget":
            cat = args.get("category")
            if cat not in CATEGORIES: return "ERROR: unknown category."
            budgets[cat] = float(args.get("limit",0) or 0); save(D)
            return f"✓ Budget for {CATEGORIES[cat]['label']} set to {usdu(budgets[cat])}/mo."
        if name == "add_goal":
            nm = str(args.get("name","")).strip()
            tgt = float(args.get("target",0) or 0)
            if not nm or tgt <= 0: return "ERROR: a name and positive target are required."
            goals.append({"id":max((g["id"] for g in goals),default=0)+1,"name":nm,
                "target":tgt,"current":float(args.get("current",0) or 0),"icon":"🎯",
                "deadline":str(date.today()+timedelta(days=365)),"linked_asset":None})
            save(D)
            return f"✓ Created goal '{nm}' (target {usdu(tgt)})."
        if name == "add_asset":
            nm = str(args.get("name","")).strip()
            val = float(args.get("value",0) or 0)
            typ = args.get("type") if args.get("type") in ASSET_TYPES else "Cash / Bank"
            if not nm or val <= 0: return "ERROR: a name and positive value are required."
            assets.append({"id":max((x["id"] for x in assets),default=0)+1,"name":nm,
                "type":typ,"value":val,"note":str(args.get("note","") or "added by AI")})
            save(D)
            return f"✓ Added asset '{nm}' ({typ}) worth {usdu(val)}."
        if name == "add_liability":
            nm = str(args.get("name","")).strip()
            bal = float(args.get("balance",0) or 0)
            typ = args.get("type") if args.get("type") in LIABILITY_TYPES else "Other Debt"
            if not nm or bal <= 0: return "ERROR: a name and positive balance are required."
            liabs.append({"id":max((x["id"] for x in liabs),default=0)+1,"name":nm,"type":typ,
                "balance":bal,"monthly_payment":float(args.get("monthly_payment",0) or 0),
                "note":str(args.get("note","") or "added by AI")})
            save(D)
            return f"✓ Added debt '{nm}' ({typ}) — balance {usdu(bal)}."
        if name == "find_cheaper_alternative":
            topic = args.get("topic")
            item = SAUDI_SAVINGS.get(topic)
            if not item: return "ERROR: unknown topic."
            spend = cat_tot.get(item.get("maps"),0) if item.get("maps") else 0
            lines = [f"{item['label']} — {item['intro']}",
                     f"(User currently spends ~{usd0u(spend)}/mo in the {item.get('maps') or 'this'} category.)",
                     "Options cheapest→premium (SAR):"]
            for o in item["options"]:
                lo,hi = o["price"]
                pr = "varies" if lo==0 and hi==0 else (f"SAR {lo:g}" if lo==hi else f"SAR {lo:,.0f}–{hi:,.0f}")
                lines.append(f"  • [{o['tag']}] {o['name']} — {pr}. {o['note']}")
            lines.append("Tips: " + " | ".join(item["tips"]))
            lines.append(f"Indicative saving if you switch: ~SAR {item.get('save_est',0):,.0f}/mo (~{usd0u(item.get('save_est',0)/SAR)} USD/mo).")
            return "\n".join(lines)
    except Exception as ex:
        return f"ERROR while running {name}: {ex}"
    return f"ERROR: unknown tool {name}."

def _claude_tools():
    return [{"name":t["name"],"description":t["description"],"input_schema":t["input_schema"]}
            for t in ai_tool_defs()]

def _gemini_tools():
    out = []
    for t in ai_tool_defs():
        params = json.loads(json.dumps(t["input_schema"]))  # deep copy
        if not params.get("properties"):
            params = None  # Gemini rejects empty object schemas
        out.append({k:v for k,v in {"name":t["name"],"description":t["description"],
                    "parameters":params}.items() if v is not None})
    return out

AGENT_SYSTEM_SUFFIX = (
    "\n\nYou can take real actions on the user's data using the provided tools "
    "(add/update/delete transactions, set budgets, add goals, add assets/debts to net worth, "
    "find cheaper alternatives). "
    "When the user asks you to add, change, remove or budget something, ACTUALLY CALL THE TOOL — "
    "do not just say you did it. After acting, briefly confirm what changed in plain language and, "
    "when relevant, suggest a next step. For 'find a cheaper alternative' requests, call "
    "find_cheaper_alternative first, then give a specific recommendation using the returned options, "
    "and offer to update the transaction amount if they want."
    "\n\nIMPORTANT: amounts are stored in USD. The user is in a city whose currency and "
    "USD exchange rate are in the snapshot. If the user gives an amount in their local "
    "currency (e.g. SAR, AED) or says 'riyal/ريال', convert it to USD using that rate "
    "(for SAR divide by 3.75) BEFORE calling a tool, and mention both figures when you confirm."
)

def ask_claude(question, history, key):
    """Agentic Claude loop with tool use. Returns (reply_text, actions_list)."""
    msgs = [{"role":"user" if r=="user" else "assistant","content":t} for r,t in history[-8:]]
    ...
    for _ in range(6):
        r = requests.post("https://api.anthropic.com/v1/messages",
            ...
            json={"model":CLAUDE_MODEL,"max_tokens":1024,
                  "system":_system_prompt()+AGENT_SYSTEM_SUFFIX,
                  "tools":_claude_tools(),"messages":msgs}, timeout=40)
        ...
        if data.get("stop_reason") == "tool_use":
            ...
            for b in content:
                if b.get("type") == "tool_use":
                    res = ai_execute(b.get("name"), b.get("input",{}))
                    actions.append(res)
    ...
    return "I took several steps — check the updated figures above.", actions

def ask_gemini(question, history, key):
    """Agentic Gemini loop with function calling. Returns (reply_text, actions_list)."""
    contents = [{"role":"user" if r=="user" else "model","parts":[{"text":t}]} for r,t in history[-8:]]
    contents.append({"role":"user","parts":[{"text":question}]})
    url = (f"https://generativelanguage.googleapis.com/v1beta/models/"
           f"{GEMINI_MODEL}:generateContent?key={key}")
    actions = []
    for _ in range(6):
        payload = {"system_instruction":{"parts":[{"text":_system_prompt()+AGENT_SYSTEM_SUFFIX}]},
                   "contents":contents,
                   "tools":[{"function_declarations":_gemini_tools()}],
        ...
        if calls:
            resp_parts = []
            for c in calls:
                res = ai_execute(c.get("name"), dict(c.get("args",{}) or {}))
                actions.append(res)
                resp_parts.append({"functionResponse":{"name":c.get("name"),"response":{"result":res}}})
...
def ask_ai(question, history):
    """Route to whichever provider has a key configured. Returns (text, actions)."""
...
def ai_quick(prompt, max_tokens=600):
    """One-shot text answer (no tools) for inline use, e.g. Save Smart cards."""
    provider, key = get_ai_provider()
    if not key:
        return None
    if provider == "claude":
        r = requests.post("https://api.anthropic.com/v1/messages",
            headers={"x-api-key":key,"anthropic-version":"2023-06-01","content-type":"application/json"},
            json={"model":CLAUDE_MODEL,"max_tokens":max_tokens,"system":_system_prompt(),
                  "messages":[{"role":"user","content":prompt}]}, timeout=60)
        if r.status_code != 200: raise RuntimeError(f"Claude API error {r.status_code}: {r.text[:160]}")
        d = r.json()
    ...
    r = requests.post(f"https://generativelanguage.googleapis.com/v1beta/models/{GEMINI_MODEL}:generateContent?key={key}",
        json={"system_instruction":{"parts":[{"text":_system_prompt()}]},
              ...
              "generationConfig":{"temperature":0.3,"maxOutputTokens":max_tokens}}, timeout=60)
    if r.status_code != 200: raise RuntimeError(f"Gemini API error {r.status_code}: {r.text[:160]}")
    d = r.json()
    return "".join(p.get("text","") for p in d["candidates"][0]["content"]["parts"] if "text" in p)

def ai_clean_transactions(txns):
    """Use the AI to tidy messy statement descriptions and re-categorise them.
    Returns a new list (same order) or None if unavailable / unparseable."""
    if not get_ai_provider()[1] or not txns:
        return None
    cats = ", ".join(CATEGORIES.keys())
    items = "\n".join(f"{i}. {t['name']} | {t['type']} | {t['amount']}" for i, t in enumerate(txns))
    prompt = ("You are tidying imported bank-statement rows. For each numbered row give a clean, "
              "human-readable merchant/description name and the single best category from this list: "
              f"{cats}.\nReturn ONLY a JSON array (no prose, no markdown fences), one object per row in "
              'the same order, e.g. [{"i":0,"name":"Starbucks","category":"food"}].\n\n' + items)
    try:
        raw = ai_quick(prompt, max_tokens=1500)
    except Exception:
        return None
    if not raw:
        return None
    ...
    for obj in data:
        ...
        if isinstance(i, int) and 0 <= i < len(out):
            if obj.get("name"): out[i]["name"] = str(obj["name"]).strip()[:48]
            if obj.get("category") in CATEGORIES: out[i]["category"] = obj["category"]
    return out

def ai_extract_receipt(image_bytes, media_type):
    """Vision: read a receipt photo and return a dict {name, amount_usd, date, category, raw}.
    Returns (data, error)."""
    provider, key = get_ai_provider()
    if not key:
        return None, "Connect an AI key (Claude or Gemini) to scan receipts."
    cats = ", ".join(CATEGORIES.keys())
    instr = ("Read this receipt and return ONLY JSON (no prose, no code fences): "
             '{"name": merchant or shop name, "amount": total paid as a number, '
             '"currency": 3-letter code shown on the receipt (e.g. SAR, AED, USD), '
             '"date": "YYYY-MM-DD" if visible else null, '
             f'"category": one of [{cats}]}}. Pick the single grand total actually paid.')
    b64 = base64.standard_b64encode(image_bytes).decode()
    try:
        if provider == "claude":
            r = requests.post("https://api.anthropic.com/v1/messages",
                headers={"x-api-key":key,"anthropic-version":"2023-06-01","content-type":"application/json"},
                json={"model":CLAUDE_MODEL,"max_tokens":400,"messages":[{"role":"user","content":[
                    {"type":"image","source":{"type":"base64","media_type":media_type,"data":b64}},
                    {"type":"text","text":instr}]}]}, timeout=60)
            if r.status_code != 200: return None, f"Claude error {r.status_code}: {r.text[:140]}"
            raw = "".join(b.get("text","") for b in r.json().get("content",[]) if b.get("type")=="text")
        else:  # gemini vision
            r = requests.post(f"https://generativelanguage.googleapis.com/v1beta/models/{GEMINI_MODEL}:generateContent?key={key}",
                json={"contents":[{"role":"user","parts":[
                    {"inline_data":{"mime_type":media_type,"data":b64}},
                    {"text":instr}]}],"generationConfig":{"temperature":0.1,"maxOutputTokens":400}}, timeout=60)
            if r.status_code != 200: return None, f"Gemini error {r.status_code}: {r.text[:140]}"
            raw = "".join(p.get("text","") for p in r.json()["candidates"][0]["content"]["parts"] if "text" in p)
    except Exception as ex:
        return None, f"Couldn't reach the AI: {str(ex)[:140]}"
    m = re.search(r"\{.*\}", raw or "", re.S)
    if not m:
        return None, "Couldn't read the receipt — try a clearer, well-lit photo."
    try:
        obj = json.loads(m.group(0))
    except Exception:
        return None, "The AI reply wasn't valid — try again with a clearer photo."
    amt = float(obj.get("amount", 0) or 0)
    curc = str(obj.get("currency", "USD") or "USD").upper()
    amt_usd = amt / CUR_RATES.get(curc, 1.0)   # store in USD
    cat = obj.get("category") if obj.get("category") in CATEGORIES else None
    nm = str(obj.get("name", "") or "Receipt").strip()[:48]
    cat = cat or detect_category(nm, "expense")
    return {"name":nm, "amount_usd":round(amt_usd,2), "amount_raw":amt, "currency":curc,
            "date":obj.get("date"), "category":cat}, None
...
/* Buttons — primary action buttons (incl. form submit) are always solid & visible.
   Use descendant selectors (not '>') so they work regardless of Streamlit's
   internal button wrapper nesting, and force the inner label colour too. */
.stButton button, .stFormSubmitButton button {{
  background:{C['accent']} !important; color:#fff !important; border:1px solid {C['accent']} !important;
  border-radius:9px !important; font-family:'Inter',sans-serif !important; font-weight:700 !important;
  font-size:.86rem !important; padding:.5rem 1rem !important; transition:all .15s ease !important;
  box-shadow:0 2px 6px {rgba(C['accent'],.3)} !important; opacity:1 !important;
}}
.stButton button p, .stButton button span, .stButton button div, .stButton button label,
.stFormSubmitButton button p, .stFormSubmitButton button span, .stFormSubmitButton button div {{
  color:#fff !important; opacity:1 !important;
}}
.stButton button:hover, .stFormSubmitButton button:hover {{ background:{C['accent2']} !important; border-color:{C['accent2']} !important; transform:translateY(-1px) !important; box-shadow:0 6px 18px {rgba(C['accent'],.4)} !important; }}
/* Sidebar's global text colour must not grey out button labels */
[data-testid="stSidebar"] .stButton button, [data-testid="stSidebar"] .stFormSubmitButton button,
[data-testid="stSidebar"] .stButton button *, [data-testid="stSidebar"] .stFormSubmitButton button * {{ color:#fff !important; }}
/* Download buttons stay subtle but clearly outlined with dark, readable text */
.stDownloadButton button {{ background:{C['surface2']} !important; border:1px solid {C['border2']} !important; box-shadow:none !important; font-weight:600 !important; }}
.stDownloadButton button, .stDownloadButton button p, .stDownloadButton button span, .stDownloadButton button div {{ color:{C['text']} !important; }}
...
with st.sidebar:
    st.markdown(f"""<div style="display:flex;gap:8px;margin-bottom:.8rem">
      <div style="flex:1;background:{rgba(C['green'],.1)};border:1px solid {rgba(C['green'],.25)};border-radius:10px;padding:.5rem .6rem">
        <div class="lbl" style="color:{C['green']}">Income/mo</div>
        <div class="mono" style="font-weight:700;color:{C['green']};font-size:1rem">{usd0(mo_inc)}</div></div>
      <div style="flex:1;background:{rgba(C['red'],.1)};border:1px solid {rgba(C['red'],.25)};border-radius:10px;padding:.5rem .6rem">
        <div class="lbl" style="color:{C['red']}">Expenses/mo</div>
        <div class="mono" style="font-weight:700;color:{C['red']};font-size:1rem">{usd0(mo_exp)}</div></div>
    </div>""", unsafe_allow_html=True)
    _, _qa_key = get_ai_provider()
    # AI-first by default; fall back to manual if there's no key or the user toggles.
    if "add_mode" not in st.session_state:
        st.session_state.add_mode = "ai" if _qa_key else "manual"
    if not _qa_key:
        st.session_state.add_mode = "manual"
    ai_on = st.session_state.add_mode == "ai" and _qa_key

    hrow = st.columns([1.6,1])
    hrow[0].markdown(f"<div style='font-weight:700;font-size:1rem;color:{C['text']};padding-top:.35rem'>Quick add</div>", unsafe_allow_html=True)
    if _qa_key:
        if ai_on:
            if hrow[1].button("✏️ Manual", key="toggle_manual", use_container_width=True):
                st.session_state.add_mode = "manual"; st.rerun()
        else:
            if hrow[1].button("✨ AI", key="toggle_ai", use_container_width=True):
                st.session_state.add_mode = "ai"; st.rerun()

    if ai_on:
        # ── AI natural-language add (primary) ──────────────────────────────
        st.markdown(f"<div class='sub' style='margin:.1rem 0 .4rem'>Just type it in plain words — the assistant adds, edits or removes it for you.</div>", unsafe_allow_html=True)
        nl = st.text_input("AI add", key="nl_add", label_visibility="collapsed",
                           placeholder="e.g. add $1000/yr car insurance")
        if st.button("✨ Add with AI", use_container_width=True, key="nl_add_btn"):
            if nl.strip():
                snapshot()
                try:
                    with st.spinner("Working…"):
                        reply, actions = ask_ai(nl.strip(), [])
                    if actions:
                        st.session_state["sidebar_ai_msg"] = ("ok", " ".join(actions))
                        st.rerun()
                    else:
                        st.session_state["sidebar_ai_msg"] = ("info", reply or "Nothing to change.")
                        st.rerun()
                except Exception as ex:
                    m = str(ex); low = m.lower()
                    if "401" in m or "403" in m or "invalid" in low or "authentication" in low:
                        fr = "API key problem — check it's valid (Settings ▸ API keys)."
                    elif "429" in m or "rate" in low or "quota" in low:
                        fr = "Assistant busy (usage limit). Try again in a minute."
                    elif "connection" in low or "timeout" in low or "network" in low:
                        fr = "Couldn't reach the AI — check your connection."
                    else:
                        fr = f"Couldn't complete that: {m[:120]}"
                    st.session_state["sidebar_ai_msg"] = ("err", fr)
                    st.rerun()
            else:
                st.warning("Type what you'd like to add.")
        msg = st.session_state.pop("sidebar_ai_msg", None)
        if msg:
            kind, text = msg
            if kind == "ok": st.success(text)
            elif kind == "err": st.error(text)
            else: st.info(text)
        st.markdown(f"<div class='sub' style='margin-top:.4rem;font-size:.7rem'>Try: “salary 4000 sar monthly”, “spent 60 on groceries”, “delete netflix”, “set food budget to 400”.</div>", unsafe_allow_html=True)
    else:
        # ── Manual form (fallback / toggle) ────────────────────────────────
        e_type = st.radio("Type", ["expense","income"], horizontal=True,
        ...
        add_label = "➕ Add income" if e_type=="income" else "➕ Add expense"
        if st.button(add_label, use_container_width=True, key="quickadd"):
            if e_name.strip() and e_amt>0:
                ...
                save(D); st.toast(f"Added {e_name.strip()}"); st.rerun()
            else:
    ...
    st.markdown(f"<div class='h-sub' style='margin-top:.6rem'>Display currency</div>", unsafe_allow_html=True)
    cur_opts = sorted(CUR_RATES.keys())
    dc = st.selectbox("Currency", cur_opts, index=cur_opts.index(DISP_CUR),
                      format_func=lambda c:f"{c}  ({cur_sym(c).strip() or c})", label_visibility="collapsed")
    if dc != DISP_CUR: D["disp_cur"]=dc; save(D); st.rerun()
    st.markdown(f"<div class='sub' style='font-size:.68rem'>Amounts are entered & stored in USD and shown in {DISP_CUR}.</div>", unsafe_allow_html=True)

    st.markdown(f"<div style='height:1px;background:{C['border']};margin:1rem 0'></div>", unsafe_allow_html=True)
    if st.session_state.get("undo"):
        if st.button("↩ Undo last change", use_container_width=True):
            try:
                st.session_state.data = json.loads(st.session_state.undo)
                save(st.session_state.data)
            except Exception: pass
            st.session_state.pop("undo", None)
            st.toast("Reverted last change"); st.rerun()
    cols = st.columns(2)
    with cols[0]:
        if st.button("Save", use_container_width=True): save(D); st.toast("Saved")
    with cols[1]:
        if st.button("Clear all", use_container_width=True):
            snapshot(); st.session_state.data = empty_data(); save(st.session_state.data); st.rerun()
    if st.button("Load sample data", use_container_width=True):
        snapshot(); st.session_state.data = sample_data(); save(st.session_state.data); st.rerun()
    ...
    if not get_ai_provider()[1]:
        ...
        if typed:
            ...
            st.rerun()
        st.markdown(f"<div class='sub'>Get a <b>Claude key</b>: sign in at <b>console.anthropic.com</b> → Settings → API keys → Create key (starts with <code>sk-ant-</code>). Or a free <b>Gemini key</b> at <b>aistudio.google.com/apikey</b>. For a deployed app, put it in Streamlit <b>Secrets</b> as ANTHROPIC_API_KEY / GEMINI_API_KEY instead of pasting it here.</div>", unsafe_allow_html=True)
# ════════════════════════════════════════════════════════════════════════════
# TABS
# ════════════════════════════════════════════════════════════════════════════
# Seven clean top-level tabs. Related screens are grouped into sub-tabs so the
# UI stays uncluttered without losing any functionality.
tOver, tAnaP, tAsk, tMoneyP, tSave, tWealthP, tRep = st.tabs(
    ["Overview","Analytics","Assistant","Money","Save Smart","Wealth","Report"])
with tAnaP:
    tAna, tIns = st.tabs(["Trends & forecast","Smart insights"])
with tMoneyP:
    tTxn, tBud, tScan, tImp = st.tabs(["Transactions","Budgets","Scan receipt","Import statement"])
with tWealthP:
    tNW, tInv, tZakat, tLive = st.tabs(["Net worth","Invest","Zakat","Live markets"])
...
# ─────────────────────────────────────────────────────────────────────────────
with tOver:
    # Friendly empty state — make it obvious how to begin.
    if not entries and not assets:
        st.markdown(f"""<div class="card" style="border:1px solid {rgba(C['accent'],.3)};background:{rgba(C['accent'],.05)};margin-bottom:1rem">
          <div style="font-size:1.05rem;font-weight:700;color:{C['text']};margin-bottom:.3rem">👋 Welcome to ClearSpend</div>
          <div style="font-size:.9rem;color:{C['text2']};line-height:1.6">
            Get started in seconds:<br>
            <b>1.</b> Use <b>Quick add</b> in the left sidebar — just type “<i>spent 60 on groceries</i>” (AI) or fill the form.<br>
            <b>2.</b> Or click <b>Load sample data</b> in the sidebar to explore with example numbers.<br>
            <b>3.</b> Add an AI key in the sidebar to let the assistant add, edit and optimise everything for you.
    ...
    with L:
        ...
        render_svg(svg_area_line([("spend", vals, C["accent"])], labels, width=620, height=210, show_dots=True), 210)
...
with tAna:
    ...
    with g1:
        ...
        render_svg(svg_bars(vals, labels, C["accent"], width=560, height=200), 200)
    ...
    render_svg(svg_heatcal(sbd, gdays, int(cyear), int(cmonth)), 320)
    mtot = sum(sbd.values())
...
with tAsk:
    ...
    st.markdown(f"<div class='sub' style='margin-top:-.6rem;margin-bottom:1rem'>Ask anything about your money — and tell it to <b>do</b> things. It reads your actual ClearSpend data and can add or change transactions, set budgets, create goals and find cheaper alternatives for you. Changes apply instantly across every tab."
    ...
    if not key:
        ...
        sugg = ["Add a $1,000/yr car insurance expense",
                "Find me a cheaper car insurance alternative",
                "Where am I overspending — and fix one thing for me",
                "Set a $400 food budget and create a $5,000 emergency goal"]
        qcols = st.columns(2)
        clicked = None
        ...
        if question:
            ...
            try:
                with st.spinner("Thinking…"):
                    reply, actions = ask_ai(question, st.session_state.chat[:-1])
                if actions:
                    done = "\n".join(actions)
                    reply = (reply + "\n\n— — —\nActions taken:\n" + done).strip()
                ...
                if "429" in msg or "rate" in low or "quota" in low:
                    friendly = "The assistant is busy (usage limit reached for now). Wait a minute and try again."
                elif "401" in msg or "403" in msg or "invalid" in low or "authentication" in low or "x-api-key" in low or "permission" in low:
                    friendly = f"API key problem — check the key is valid and active. Details: {msg[:160]}"
                elif "model" in low or "404" in msg or "400" in msg:
                    friendly = f"Model/request problem. Details: {msg[:160]}"
                elif "connection" in low or "timeout" in low or "resolve" in low or "network" in low:
                    friendly = "Couldn't reach the AI service — check your internet connection and try again."
                else:
                    friendly = f"Something went wrong. Details: {msg[:180]}"
            ...
            with st.expander(f"{item['emoji']}  {item['label']}", expanded=(choice!='All areas')):
                ...
                if item.get("save_est"):
                    st.markdown(f"""<div style="margin-top:.7rem;padding:.6rem .9rem;border-radius:10px;
                        ...
                        <span class="sub" style="margin-left:6px">(≈ {usd0u(item['save_est']/SAR)}/mo · {usd0u(item['save_est']*12/SAR)}/yr USD) if you switch to a cheaper option</span>
                    </div>""", unsafe_allow_html=True)

                # ── AI-powered personalised alternative ──────────────────────
                _, _aikey = get_ai_provider()
                if _aikey:
                    st.markdown("<div style='height:.5rem'></div>", unsafe_allow_html=True)
                    if st.button(f"✨ Find me a cheaper alternative", key=f"ai_alt_{key}"):
                        try:
                            with st.spinner("Asking the assistant…"):
                                spend_line = (f"I currently spend about {usd0u(spend)}/mo on {item['label'].lower()}. "
                                              if cat and spend>0 else "")
                                rec = ai_quick(
                                    f"{spend_line}Using these real Saudi options for {item['label']}:\n"
                                    + "\n".join(f"- {o['name']}: "
                                        + ("varies" if o['price']==(0,0) else (f"SAR {o['price'][0]:g}" if o['price'][0]==o['price'][1] else f"SAR {o['price'][0]:,.0f}–{o['price'][1]:,.0f}"))
                                        + f" ({o['note']})" for o in item['options'])
                                    + "\n\nRecommend the best cheaper option for me specifically, estimate my monthly/yearly saving in SAR and USD, and give me 2–3 concrete next steps. Be concise.")
                            st.session_state[f"ai_alt_res_{key}"] = rec or "No suggestion returned."
                        except Exception as ex:
                            st.session_state[f"ai_alt_res_{key}"] = f"Couldn't reach the assistant: {str(ex)[:160]}"
                    if st.session_state.get(f"ai_alt_res_{key}"):
                        st.markdown(f"""<div style="margin-top:.6rem;padding:.75rem .95rem;border-radius:11px;
                            background:{rgba(C['accent'],.07)};border:1px solid {rgba(C['accent'],.25)}">
                            <div class="lbl" style="color:{C['accent']};margin-bottom:4px">✨ AI recommendation</div>
                            <div style="font-size:.85rem;color:{C['text2']};line-height:1.55">{st.session_state[f'ai_alt_res_{key}'].replace(chr(10),'<br>')}</div>
                          </div>""", unsafe_allow_html=True)
...
with tInv:
    ...
    if spare<=0:
        ...
        render_svg(svg_area_line([("Invested",with_inv,C["accent"]),("Cash only",cash_only,C["text3"])],
                    labels, width=900, height=230, show_dots=False), 230)
...
with tNW:
    ...
    render_svg(svg_area_line([("Base",base,C["accent"]),("Optimistic",opt,C["green"]),("Conservative",con,C["amber"])],
                [str(y) for y in range(pyrs+1)], width=900, height=230), 230)
...
with tImp:
    ...
    st.markdown(f"<div class='sub' style='margin-bottom:.8rem'>Upload a bank statement PDF. ClearSpend reads it locally, extracts each transaction, auto-categorises it, and previews before you import. With an AI key connected you can also tidy messy descriptions and fix categories in one click.</div>", unsafe_allow_html=True)
    ...
    if up is not None:
        ...
        if err:
            ...
            # Reuse an AI-cleaned version if it matches this exact upload.
            sig = str(hash(tuple((t["name"], round(t["amount"],2), t["date"]) for t in txns)))
            if st.session_state.get("pdf_sig")==sig and st.session_state.get("pdf_clean"):
                txns = st.session_state["pdf_clean"]
            ...
            st.markdown("<div style='height:.5rem'></div>", unsafe_allow_html=True)
            if get_ai_provider()[1]:
                ca = st.columns([1.4,2])
                if ca[0].button("✨ Clean up names & categories with AI"):
                    with st.spinner("Tidying with AI…"):
                        cleaned = ai_clean_transactions(txns)
                    if cleaned:
                        st.session_state["pdf_clean"]=cleaned; st.session_state["pdf_sig"]=sig
                        st.rerun()
                    else:
                        ca[1].info("Couldn't auto-clean (no key or unexpected reply). Using the basic parse.")
...
# ─────────────────────────────────────────────────────────────────────────────
# SCAN RECEIPT  (AI vision → auto-add)
# ─────────────────────────────────────────────────────────────────────────────
with tScan:
    st.markdown(f"<div class='h-sec'>Scan a receipt</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='sub' style='margin-top:-.6rem;margin-bottom:1rem'>Snap or upload a photo of a receipt — the AI reads the merchant, total and date, picks a category, converts it to your stored currency (USD), and adds it. No typing.</div>", unsafe_allow_html=True)
    _sp, _sk = get_ai_provider()
    if not _sk:
        st.warning("Connect an AI key in the sidebar to scan receipts (uses image vision).")
    else:
        src = st.radio("Source", ["📷 Camera","📁 Upload"], horizontal=True, label_visibility="collapsed")
        img = None; mt = "image/jpeg"
        if src == "📷 Camera":
            cam = st.camera_input("Point at the receipt")
            if cam: img = cam.getvalue(); mt = cam.type or "image/jpeg"
        else:
            upf = st.file_uploader("Receipt image", type=["png","jpg","jpeg","webp"])
            if upf: img = upf.read(); mt = upf.type or "image/jpeg"
        if img is not None and st.button("✨ Read receipt"):
            with st.spinner("Reading the receipt…"):
                data, err = ai_extract_receipt(img, mt)
            if err: st.error(err)
            else: st.session_state["receipt"] = data; st.rerun()

        rec = st.session_state.get("receipt")
        if rec:
            cur_note = f" · originally {rec['currency']} {rec['amount_raw']:,.2f}" if rec.get("currency") not in (None,"USD") else ""
            st.success(f"Found: {rec['name']} — {usd(rec['amount_usd'])}{cur_note}. Check & add below.")
            f = st.columns([2,1,1.3])
            rn = f[0].text_input("Description", value=rec["name"], key="rc_n")
            ramt = f[1].number_input("Amount (USD)", min_value=0.0, value=float(rec["amount_usd"]), step=1.0, key="rc_a")
            ckz = list(CATEGORIES.keys())
            ridx = ckz.index(rec["category"]) if rec.get("category") in ckz else ckz.index("other")
            rcat = ckz[f[2].selectbox("Category", range(len(ckz)), index=ridx,
                     format_func=lambda i:f"{CATEGORIES[ckz[i]]['emoji']} {CATEGORIES[ckz[i]]['label']}", key="rc_c")]
            try: rdate = datetime.strptime(rec.get("date") or "", "%Y-%m-%d").date()
            except: rdate = date.today()
            rdate = st.date_input("Date", value=rdate, key="rc_d")
            ac = st.columns(2)
            if ac[0].button("➕ Add this expense", key="rc_add", use_container_width=True):
                if (rn.strip() and ramt>0):
                    snapshot()
                    entries.append({"id":max((x["id"] for x in entries),default=0)+1,"name":rn.strip(),
                        "amount":float(ramt),"type":"expense","ptype":"single","category":rcat,
                        "date":str(rdate),"note":"scanned receipt","linked_goal":None})
                    save(D); st.session_state.pop("receipt",None); st.toast("Added from receipt"); st.rerun()
                else:
                    st.error("Need a description and amount.")
            if ac[1].button("Discard", key="rc_clear", use_container_width=True):
                st.session_state.pop("receipt",None); st.rerun()

# ─────────────────────────────────────────────────────────────────────────────
# ZAKAT CALCULATOR
# ─────────────────────────────────────────────────────────────────────────────
with tZakat:
    Z = D.setdefault("zakat", {"hawl_start":None,"gold_price_g":80.0,"payments":[]})
    rate = CUR_RATES.get(DISP_CUR, 1.0)
    st.markdown(f"<div class='h-sec'>Zakat calculator</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='sub' style='margin-top:-.6rem;margin-bottom:1rem'>Zakat is 2.5% of your zakatable wealth (cash, investments, gold, crypto) once it's been held for a full lunar year and exceeds the <b>nisab</b> threshold. This pulls from your tracked assets. For your exact situation, confirm with a knowledgeable scholar.</div>", unsafe_allow_html=True)

    s = st.columns(3)
    gold_disp = s[0].number_input(f"Gold price per gram ({DISP_CUR})", min_value=0.0,
                                  value=float(Z.get("gold_price_g",80.0)*rate), step=1.0)
    gold_usd_g = (gold_disp/rate) if rate else gold_disp
    try: hs = datetime.strptime(Z.get("hawl_start") or "", "%Y-%m-%d").date()
    except: hs = date.today()
    hawl_start = s[1].date_input("Wealth-year (hawl) start", value=hs)
    incl = s[2].multiselect("Zakatable asset types", ASSET_TYPES,
            default=[t for t in ["Cash / Bank","Investments","Crypto"] if t in ASSET_TYPES])
    if round(gold_usd_g,4)!=Z.get("gold_price_g") or str(hawl_start)!=Z.get("hawl_start"):
        Z["gold_price_g"]=round(gold_usd_g,4); Z["hawl_start"]=str(hawl_start); save(D)

    s2 = st.columns(2)
    manual_disp = s2[0].number_input(f"Other zakatable wealth — cash on hand, gold ({DISP_CUR})",
                                     min_value=0.0, value=float(Z.get("manual",0.0)*rate), step=10.0)
    debts_disp  = s2[1].number_input(f"Deductible short-term debts ({DISP_CUR})",
                                     min_value=0.0, value=float(Z.get("debts",0.0)*rate), step=10.0)
    manual_usd = (manual_disp/rate) if rate else manual_disp
    debts_usd  = (debts_disp/rate) if rate else debts_disp
    if round(manual_usd,2)!=Z.get("manual") or round(debts_usd,2)!=Z.get("debts"):
        Z["manual"]=round(manual_usd,2); Z["debts"]=round(debts_usd,2); save(D)

    asset_z = sum(a["value"] for a in assets if a["type"] in incl)
    zbase = max(0.0, asset_z + manual_usd - debts_usd)
    nisab = 85.0 * gold_usd_g
    above = zbase >= nisab
    due = zbase*0.025 if above else 0.0
    due_date = hawl_start + timedelta(days=354)
    dleft = (due_date - date.today()).days

    k = st.columns(4)
    kpi(k[0], "Zakatable wealth", usd0(zbase), "assets + cash − debts", C["accent"])
    kpi(k[1], "Nisab (85g gold)", usd0(nisab), "above this → zakat is due", C["amber"])
    kpi(k[2], "Zakat (2.5%)", usd0(due), "of zakatable wealth", C["green"] if above else C["text3"])
    kpi(k[3], "Wealth-year", (f"{dleft}d to go" if dleft>0 else "Due now"),
        f"ends {due_date.isoformat()}", C["blue"])

    st.markdown("<div style='height:.6rem'></div>", unsafe_allow_html=True)
    L, R = st.columns([1.3,1], gap="medium")
    with L:
        st.markdown(f"<div class='card'>", unsafe_allow_html=True)
        st.markdown(f"<div class='h-sec'>What counts</div>", unsafe_allow_html=True)
        rows = [(a["name"], a["type"], a["value"]) for a in assets if a["type"] in incl]
        if manual_usd>0: rows.append(("Other (cash/gold)","Manual",manual_usd))
        if not rows:
            st.markdown(f"<div class='sub'>No zakatable assets yet — add cash, investments or crypto in Net worth, or enter an amount above.</div>", unsafe_allow_html=True)
        for nm,ty,v in rows:
            st.markdown(f"""<div style="display:flex;justify-content:space-between;padding:.35rem 0;border-bottom:1px solid {C['line']};font-size:.85rem">
              <span style="font-weight:600">{nm} <span class='sub'>· {ty}</span></span>
              <span class="mono" style="font-weight:700;color:{C['accent']}">{usd0(v)}</span></div>""", unsafe_allow_html=True)
        if debts_usd>0:
            st.markdown(f"""<div style="display:flex;justify-content:space-between;padding:.35rem 0;font-size:.85rem">
              <span style="font-weight:600;color:{C['red']}">− Deductible debts</span>
              <span class="mono" style="font-weight:700;color:{C['red']}">−{usd0(debts_usd)}</span></div>""", unsafe_allow_html=True)
        st.markdown(f"<div class='divider'></div><div style='display:flex;justify-content:space-between;font-weight:700'><span>Zakatable total</span><span class='mono' style='color:{C['accent']}'>{usd0(zbase)}</span></div>", unsafe_allow_html=True)
        if not above and nisab>0:
            st.markdown(f"<div class='sub' style='margin-top:.5rem;color:{C['amber']}'>Below nisab by {usd0(nisab-zbase)} — no zakat is due yet.</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    with R:
        st.markdown(f"<div class='card'>", unsafe_allow_html=True)
        st.markdown(f"<div class='h-sec'>Dates</div>", unsafe_allow_html=True)
        st.markdown(f"""<div style="font-size:.86rem;line-height:1.9">
          <div style="display:flex;justify-content:space-between"><span class='sub'>Today</span><span>{hijri_str(date.today())}</span></div>
          <div style="display:flex;justify-content:space-between"><span class='sub'>Hawl start</span><span>{hijri_str(hawl_start)}</span></div>
          <div style="display:flex;justify-content:space-between"><span class='sub'>Zakat due</span><span style="font-weight:700;color:{C['accent']}">{hijri_str(due_date)}</span></div>
        </div>""", unsafe_allow_html=True)
        st.markdown(f"<div class='sub' style='margin-top:.4rem;font-size:.7rem'>Hijri dates are approximate (tabular calendar) — confirm the exact day locally.</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div style='height:.6rem'></div>", unsafe_allow_html=True)
    st.markdown(f"<div class='h-sec'>Log a zakat payment</div>", unsafe_allow_html=True)
    pc = st.columns([2,1])
    pay_disp = pc[0].number_input(f"Amount ({DISP_CUR})", min_value=0.0,
                                  value=float(round(due*rate,2)), step=10.0, key="zpay_amt")
    pc[1].markdown("<div style='height:1.7rem'></div>", unsafe_allow_html=True)
    if pc[1].button("Log payment", use_container_width=True):
        if pay_disp>0:
            snapshot()
            Z.setdefault("payments",[]).append({"amount":round((pay_disp/rate) if rate else pay_disp,2),
                                                "date":date.today().isoformat()})
            save(D); st.toast("Zakat payment logged"); st.rerun()
    pays = Z.get("payments",[])
    if pays:
        paid_total = sum(p.get("amount",0) for p in pays)
        st.markdown(f"<div class='sub' style='margin:.3rem 0'>Logged payments — total {usd0(paid_total)}</div>", unsafe_allow_html=True)
        for i,p in enumerate(sorted(pays, key=lambda x:x.get("date",""), reverse=True)):
            cc = st.columns([6,1])
            cc[0].markdown(f"""<div style="display:flex;justify-content:space-between;padding:.3rem 0;border-bottom:1px solid {C['line']};font-size:.85rem">
              <span class='sub'>{p.get('date','')} · {hijri_str(datetime.strptime(p['date'],'%Y-%m-%d').date()) if p.get('date') else ''}</span>
              <span class="mono" style="font-weight:700;color:{C['green']}">{usd0(p.get('amount',0))}</span></div>""", unsafe_allow_html=True)
            if cc[1].button("✕", key=f"zpd{i}"):
                Z["payments"].remove(p); save(D); st.rerun()
