...
def build_finance_context():
    ...
    if entries:
        ...
        for e in sorted(entries, key=lambda x:-monthly_eq(x["amount"],x["ptype"]))[:40]:
            lines.append(f"  #{e['id']} {e['name']} {usd(e['amount'])} {e['ptype']} "
                         f"{e['category']} {e['type']}")
    return "\n".join(lines)
...
def ai_execute(name, args):
    ...
    try:
        if name == "add_transaction":
            ...
            save(D)
            return f"✓ Added {typ} '{nm}' — {usd(amt)} ({PAYMENT_TYPES[freq]['label']}, {CATEGORIES[cat]['label']})."
        if name == "update_transaction":
            ...
            if args.get("amount") is not None:
                tgt["amount"] = float(args["amount"]); changed.append(f"amount→{usd(tgt['amount'])}")
            if args.get("name"):
        ...
        if name == "delete_transaction":
            ...
            return f"✓ Deleted '{tgt['name']}' ({usd(tgt['amount'])})."
        if name == "set_budget":
            cat = args.get("category")
            if cat not in CATEGORIES: return "ERROR: unknown category."
            budgets[cat] = float(args.get("limit",0) or 0); save(D)
            return f"✓ Budget for {CATEGORIES[cat]['label']} set to {usd(budgets[cat])}/mo."
        if name == "add_goal":
            nm = str(args.get("name","")).strip()
            ...
            save(D)
            return f"✓ Created goal '{nm}' (target {usd(tgt)})."
        if name == "find_cheaper_alternative":
            ...
            lines = [f"{item['label']} — {item['intro']}",
                     f"(User currently spends ~{usd0(spend)}/mo in the {item.get('maps') or 'this'} category.)",
            ...
            lines.append(f"Indicative saving if you switch: ~SAR {item.get('save_est',0):,.0f}/mo (~{usd0(item.get('save_est',0)/SAR)}/mo).")
            return "\n".join(lines)
    except Exception as ex:
...
AGENT_SYSTEM_SUFFIX = (
    ...
    "(add/update/delete transactions, set budgets, add goals, find cheaper alternatives). "
...
def ai_quick(prompt):
    ...
    if provider == "claude":
        r = requests.post("https://api.anthropic.com/v1/messages",
            headers={"x-api-key":key,"anthropic-version":"2023-06-01","content-type":"application/json"},
            json={"model":CLAUDE_MODEL,"max_tokens":600,"system":_system_prompt(),
    ...
    r = requests.post(f"https://generativelanguage.googleapis.com/v1beta/models/{GEMINI_MODEL}:generateContent?key={key}",
        json={"system_instruction":{"parts":[{"text":_system_prompt()}]},
              ...
              "generationConfig":{"temperature":0.5,"maxOutputTokens":600}}, timeout=40)
...
with st.sidebar:
    ...
    with cols[1]:
        if st.button("Clear all", use_container_width=True):
            st.session_state.data = empty_data(); save(st.session_state.data); st.rerun()
    if st.button("Load sample data", use_container_width=True):
        st.session_state.data = sample_data(); save(st.session_state.data); st.rerun()
                    ...
                    if st.button(f"✨ Find me a cheaper alternative", key=f"ai_alt_{key}"):
                        try:
                            with st.spinner("Asking the assistant…"):
                                spend_line = (f"I currently spend about {usd0(spend)}/mo on {item['label'].lower()}. "
